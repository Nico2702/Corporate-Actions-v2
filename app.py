...
# ── API keys (silently loaded from .streamlit/secrets.toml) ───────────────────

def _get_secret(key: str, default: str = "") -> str:
    """Read a value from .streamlit/secrets.toml. Returns default if the file
    or key is missing, so the app still runs (fetch calls will simply fail)."""
    try:
...
edi_key     = _get_secret("edi_key")
factset_key = _get_secret("factset_key")
...
def render_factset_tab():
    ...
    with tab1:
        ...
        display_cols = [
            ...
            "Sub_Price", "Sub_Currency", "Sub_Ratio",
            "ECA_Stock_Ratio", "ECA_Stock_Terms",
            "Deal_Type", "MA_Cash_Terms", "MA_Cash_Terms_Currency",
...
def render_validation_tab():
    ...
    for r in filtered:
        ...
        summary_rows.append({
            ...
            "Sub_Price":        _val(r, "Sub_Price"),
            "Sub_Currency":     _val(r, "Sub_Currency"),
            "Sub_Ratio":        _val(r, "Sub_Ratio"),
            "Stock_Div_Pct":    _val(r, "Stock_Div_Pct"),
            "Split_Ratio":      _val(r, "Split_Ratio"),
            "Ex_Date":          r["exdt"],
